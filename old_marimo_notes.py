...
def _(mo):
    mo.md(
        ...
        or can be rearranged to have the **standard form**:  $$\begin{equation}x + b = 0 \end{equation}$$
        ...
        $$\begin{align} ax &= -b \\ x &= -b/a \end{align}$$
...
@app.cell
def _(mo):
    # Demonstrating the zero of a linear function

    def linear_func(x: float, a: float, b: float):
        return a*x + b

    def linear_zero(a: float, b: float):
        return - b/a
    ...
    coeffs = mo.ui.array([
        mo.ui.slider(
            ...
            step=1/1000, 
            value=1, 
            label=f"Value of {['$a$', '$b$'][i]}:", 
            show_value=True, 
            orientation="horizontal"
        ) for i in range(2)
    ])

    mo.md(f"Choose a values for the numbers $a$ and $b$: {coeffs}")
    return coeffs, linear_func, linear_zero


@app.cell
def _(coeffs, linear_func, linear_zero, np, plt):
    a_coeff = coeffs.value[0]
    b_coeff = coeffs.value[1]


    x_vals = np.linspace(-20,20)
    y_vals = linear_func(x_vals, a_coeff, b_coeff)

    def draw_linear_function(x_vals, y_vals):
        # caption: Here the blue line indicates the function $f(x) = ax+b$ and the green dashed line tells us where the function is at a zero $x=-b/a$
        # label: linear_func
        fig, ax = plt.subplots()
        ax.grid(True, color = 'silver') # Setting a silver grid


        # Draw axes
        ax.plot(0*x_vals, np.linspace(-11*np.abs(np.min(y_vals)), 11*np.abs(np.max(y_vals))), color = 'black')
        ax.plot(np.linspace(-1.1*np.abs(np.min(x_vals)), 1.1*np.abs(np.max(x_vals))), 0*y_vals, color = 'black')


        # Draw linear function
        ax.plot(x_vals, y_vals, color = 'blue', label = f'Linear function: $y=a\\cdot x + b$')
        ax.vlines(linear_zero(a_coeff, b_coeff), -11*np.abs(np.min(y_vals)),22, color = 'green', linestyle = 'dashed', label = f'$x = -b/a =$ {b_coeff/a_coeff:.3f}')

        # Setting x and y limits & label axes
    ...
    draw_linear_function(x_vals, y_vals)
    return a_coeff, b_coeff, draw_linear_function, x_vals, y_vals
